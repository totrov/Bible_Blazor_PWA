...
namespace Bible_Blazer_PWA
{
    internal class InMemoryBibleServiceFetchStrategy : IBibleServiceFetchStrategy
    {
        ...
        public int GetBookIdByShortName(string bookShortName)
        {
            return _books.Where(b => b.ShortName == bookShortName).Select(b => b.Id).First();
        }
        ...
        public IEnumerable<BibleService.Verse> GetVerses(int bookId, int chapter, int fromVerse, int? toVerse)
        {
        ...
        int IBibleServiceFetchStrategy.GetBookIdByShortName(string bookShortName)
        {
            return _books.Where(b => b.ShortName == bookShortName).Select(b => b.Id).First();
        }
    }
